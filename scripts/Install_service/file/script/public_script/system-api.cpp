...
// 添加一天系统运行日志 /opt/runtimeLogs.sh 0:false 1:true
bool addLog(string level, string service, string message) {
    string cmd="bash /opt/runtimeLogs.sh \""+level+"\" \""+service+"\" \""+message+"\" |grep 'INSERT 0 1'|wc -l";
    string res=exec(cmd);
    int rest=atoi(res.c_str());
    return rest==1;
}
